...
describe('PolicyListItemComponent', () => {
  ...
  beforeEach(() => {
    ...
    component.data = JSON.parse(JSON.stringify(policyObj));
  ...
  it('should show null policy column', fakeAsync(() => {
    delete component.data.repo.name;
    fixture.detectChanges();
    tick();
  ...
  it('should show null broker column', fakeAsync(() => {
    delete component.data.broker.name;
    fixture.detectChanges();
    tick();
  ...
  it('should show null task column', fakeAsync(() => {
    delete component.data.task.name;
    fixture.detectChanges();
    tick();
  ...
  it('should show bar color', fakeAsync(() => {
    component.data.max_count = 100;
    component.data.nodes.count = 100;
    expect(component.barColor()).toBe('danger');
    component.data.nodes.count = 101;
    expect(component.barColor()).toBe('danger');
    component.data.nodes.count = 90;
    expect(component.barColor()).toBe('warning');
    component.data.nodes.count = 85;
    expect(component.barColor()).toBe('warning');
    component.data.nodes.count = 84;
    expect(component.barColor()).toBe('info');
    component.data.nodes.count = 10;
    expect(component.barColor()).toBe('info');
  }));
